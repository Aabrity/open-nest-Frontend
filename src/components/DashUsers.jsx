...
const UsersTablePage = () => {
  // State for users, showMore button, modal visibility, and user ID to delete
    ...
    const fetchUsers = async () => {
      try {
        ...
        if (res.ok) {
          ...
          toast.error("Failed to fetch users."); // Show error message using toast
        }
      } catch (error) {
        toast.error("Failed to fetch users."); // Show error message using toast
      }
  ...
  return (
    <div className="p-6 bg-white w-full dark:bg-gray-800 ">
      ...
      {/* Conditional rendering: show table if users exist */}
      {users.length > 0 ? (
        <>
          ...
          {/* Show "Show more" button if there are more users to load */}
          {showMore && (
