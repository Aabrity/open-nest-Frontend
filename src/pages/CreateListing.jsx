...
export default function CreateListing() {
    ...
    if (files.length > 0 && files.length + formData.imageUrls.length < 7) {
      ...
      Promise.all(promises)
        .then((base64Images) => {
          ...
          toast.success('Images uploaded successfully!'); // Show success message using toast
          setUploading(false);
        })
        .catch((err) => {
          toast.error('Image upload failed. Please try again.'); // Show error message using toast
          setUploading(false);
        });
    } else {
      toast.error('You can only upload 6 images per listing.'); // Show error message using toast
      setUploading(false);
    }
  };
  ...
  const handleRemoveImage = (index) => {
    ...
    toast.success('Image removed successfully!'); // Show success message using toast
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formData.imageUrls.length < 1) {
        toast.error('You must upload at least one image.'); // Show error message using toast
        return;
      }
      if (+formData.regularPrice < +formData.discountPrice) {
        toast.error('Discount price must be lower than regular price.'); // Show error message using toast
        return;
      }
      setLoading(true);
      const res = await fetch('/api/listing/create', {
      ...
      toast.success('Listing created successfully!'); // Show success message using toast
      navigate(`/listing/${data._id}`);
    } catch (error) {
      toast.error('Failed to create listing. Please try again.'); // Show error message using toast
      setLoading(false);
    }
  };
